...
function PurchaseInvoice() {
  ...
  // Fetch Masters and Purchases on Mount
  useEffect(() => {
    fetchVendors();
    fetchItems();
  ...
  const fetchPurchaseInvoices = async () => {
    ...
    try {
      const res = await api.get("/transactions/purchase-invoices");
      setSavedPurchases(res.data);
    } catch (err) {
  ...
  const updateItemDetails = (id, dbItem) => {
    const updatedItems = purchaseItems.map((item) => {
      if (item.id === id) {
        ...
        const rate = Number(dbItem.purchasePrice) || 0;
  ...
  const grandTotal = Math.round(
    purchaseItems.reduce((total, item) => total + Number(item.amount), 0) * 100
  ) / 100;
  ...
  const savePurchase = async () => {
    ...
    if (!vendorId) {
      setMessage("Vendor selection is required");
      setMessageType("error");
      return;
    }
    ...
    try {
      const payload = {
        ...
        invoiceNo: customInvoiceNo || undefined,
      ...
      await api.post("/transactions/purchase-invoices", payload);

      // Force refresh of Master Items to get updated database stock levels!
      await fetchItems(true);

      // Reload saved purchases table
      fetchPurchaseInvoices();
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-bold mb-6 text-gray-800">Purchase Invoice</h1>
      ...
      <div className="bg-white p-6 rounded-xl shadow-md border border-gray-100">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div>
            ...
            <input
              type="text"
              placeholder="e.g. VEN-9871 (Optional)"
              value={customInvoiceNo}
              onChange={(e) => setCustomInvoiceNo(e.target.value)}
              className="border border-gray-200 p-3 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent font-mono transition"
            />
          </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-md border border-gray-100 overflow-x-auto">
        <h2 className="text-xl font-bold mb-4 text-gray-800">Purchase Items</h2>

        <table className="w-full min-w-[700px] mb-4">
          <thead className="bg-gray-50 text-gray-700 border-b border-gray-200">
            <tr>
              <th className="p-3 text-left font-semibold text-sm">Item</th>
              <th className="p-3 text-left font-semibold text-sm w-32">Current Stock</th>
              <th className="p-3 text-left font-semibold text-sm w-28">Qty</th>
              <th className="p-3 text-left font-semibold text-sm w-36">Purchase Rate</th>
                    ...
                    customRender={(opt) => (
                      <div className="flex justify-between w-full">
                        ...
                        <span className="text-sm text-gray-500">
                          Stock: {opt.openingStock} | Cost: ₹{opt.purchasePrice}
                        </span>
                      </div>
                    )}
        ...
        <div className="bg-gray-50 p-6 rounded-xl border border-gray-100 mt-6 shadow-inner">
          <h2 className="text-lg font-bold mb-4 text-gray-800">Purchase Summary</h2>
      ...
      <div className="bg-white p-6 rounded-xl shadow-md border border-gray-100">
        ...
        {loadingPurchases ? (
          <div className="text-center py-6 text-gray-500">Loading purchase history...</div>
        ) : savedPurchases.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 text-gray-700 border-b border-gray-200">
                <tr>
                  <th className="p-3 text-left font-semibold text-sm">Purchase No</th>
                  <th className="p-3 text-left font-semibold text-sm">Date</th>
                  <th className="p-3 text-left font-semibold text-sm">Vendor</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {savedPurchases.map((purchase) => (
                  <tr key={purchase.id} className="hover:bg-gray-50/50 transition">
                    ...
                    <td className="p-3 text-gray-600">{purchase.totalItems}</td>
                    <td className="p-3 text-gray-600">{purchase.totalQty}</td>
          ...
          <div className="text-center py-6 text-gray-400">No purchases saved yet</div>
        )}
      </div>

      <CommonModal isOpen={showSaveModal} title="Purchase Invoice Saved">
        <p className="mb-4 text-gray-600">
          The Purchase Invoice has been successfully recorded. Stock levels have been incremented.
        </p>
